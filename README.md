...
***Disclaimer**: this is an experimental project to improve and showcase my current skills in cloud computing and data engineering and is not in any way meant to represent a full software solution for applications beyond personal use.*
...
3. [Code Demonstration](#Code-Demonstration)
4. [Brief Code Documentation](#Brief-Code-Documentation)
...
1. If you do not have an AWS account, you can sign up [here](https://aws.amazon.com/free) for free.
...
7. Run the script to set up the cloud infrastructure and start the interactive session:
   ...
   **Note**: it can take a while for the database to be active (usually somewhere between 5 and 15 minutes) and for it to be deleted again later, to save you the trouble of deleting them manually via the AWS Management Console (web interface).
...
**WARNING**: when exiting without the *delete* command, you will have to delete all created AWS services manually! Without deleting the infrastructure (either by using *delete*, or manually), the warehouse will cause **ongoing incurred costs to your AWS account**!

*In case of any errors while setting up the cloud infrastructure, e.g. if your credentials are invalid or if your internet connection is unstable, the program will provide error messages to assist you in fixing the problem.*
...
At this stage, the processed data can be extracted from the database using SQL queries for further analysis and visualisation (planned for a future version). 

Afterwards, the infrastructure can be removed by using the *delete* command. This first shuts down and deletes the RDS instance (in the current version, no backups or snapshots are saved, so the processed data is lost), followed by the other AWS services in the data warehouse. 
...
*Note that this section does not serve as full documentation of the code and is (in its current state) only meant to give a brief overview of the different components of this project.*
...
A short script that uses the requests library to download the CSV dataset from the EEA website. See [Data Specifications](#Data-Specifications). 
...
Contains JSON files specifying the configurations of IAM policies and security groups used in this project.
...
- Add option to save backups of the database
...
- Replace infrastructure to enable completely free use of this code
...
I appreciate any feedback and comments on this project!
